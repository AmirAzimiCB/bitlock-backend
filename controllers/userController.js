...
export const updateBankingInfo = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ status: "Success", result: "Saved", user })
      .end();
  } catch (err) {
    console.log(err);
    return res.status(500).json(err).end();
...
export const applyLoan = async (req, res) => {
  try {
    ...
    // If file was uploaded, set the file path to articles_of_incorporation field
    if (req.file) {
      loanData.articles_of_incorporation = `/uploads/${req.file.filename}`;
    }
    ...
    sendAdminEmail(
      user,
      req.body,
      "loan_notification.ejs",
      "New loan has applied on BitLoc"
    );
...
export const getMyPendingLoans = async (req, res) => {
  let loans = await Loan.find({ user: req.params.id, approved: null }).populate(
    "payments"
  );
  return res.status(200).json({ status: "Success", loans }).end();
};
  ...
  try {
    ...
    if (req.body?.id != undefined) {
      ...
      sendEmail(
        req?.body?.email,
        req?.body,
        "wallet_address_change.ejs",
        "Bitcoin Wallet Address Change - Important Update"
      );
    } else {
...
export const getMyWallets = async (req, res) => {
  let walletGroups = await WalletGroup.find({ user: req.params.id }).populate(
    "wallets"
  );
...
export const changePassword = async (req, res) => {
  try {
    ...
    if (passwordCorrect) {
      ...
      return res
        .status(200)
        .json({ status: "Success", result: "Password Changed" })
        .end();
    } else {
      return res
        .status(200)
        .json({ status: "Failure", result: "Invalid Password" })
        .end();
    }
  } catch (err) {
    console.log(err);
...
export const get_2fa_secret = async (req, res) => {
  ...
  return res
    .status(200)
    .json({ status: "Success", result: "Verify Secret", secret: secret })
    .end();
};
export const verify_2fa_code = async (req, res) => {
  ...
  if (verified) {
    ...
    return res
      .status(200)
      .json({ status: "Success", result: "2 Factor Authentication Enabled" })
      .end();
  } else {
    return res
      .status(200)
      .json({ status: "failure", result: "Unable to verify" })
      .end();
  }
};
...
export const disable_2fa = async (req, res) => {
  ...
  if (verified) {
    ...
    return res
      .status(200)
      .json({ status: "Success", result: "2 Factor Authentication Disabled" })
      .end();
  } else {
    return res
      .status(200)
