  ...
  try {
    ...
    if (existing_user.length > 0) {
      ...
      sendSms(
        ...
        `Welcome and thank you for signing up to BitLoc. Your 4 Digit OTP Code is ${genOtp}`
      );
      sendEmail(
        req?.body?.email,
        newUser,
        "welcomeEmail.ejs",
        "Welcome and thank you for signing up to BitLoc"
      );
      sendEmail(
        "info@bitloc.io",
        newUser,
        "userNotification.ejs",
        "New Bitloc Account Created"
      );
      res.status(200).json({
        status: "Success",
        result: {
...
export const loginUser = async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(200)
        .json({ status: "Failure", result: "Incorrect Email or Password" });
    } else {
      if (!user.is_verified) {
        let genOtp = generateOTP(4);
...
export const verify_otp = async (req, res) => {
  ...
  if (verified) {
    ...
    return res
      .status(200)
      .json({ status: "failure", error: "Unable to verify" })
      .end();
  }
};
...
export const forgotPassword = async (req, res) => {
  try {
    ...
    sendEmail(
      ...
      user,
      "forgotPassword.ejs",
      "Forgot Password - Reset Your BitLoc Account Password"
    );
  } catch (error) {
  ...
  return res
    ...
    .json({
      status: "Success",
      result: "Email sent to the user for password update",
    })
    .end();
};
...
export const checkEmailPhone = async (req, res) => {
  if (req.body?.email != undefined) {
    ...
    if (user?._id != undefined) {
      return res
        .status(200)
        .json({ status: "Failure", result: "Already Exists" })
        .end();
    }
  } else {
...
export const resetPassword = async (req, res) => {
  try {
    ...
    if (existing_user) {
      ...
      return res
        .status(200)
        .json({ status: "Failure", result: "The link is Expired" })
        .end();
    }
  } catch (err) {
    console.log(err);
...
export const verifyOTP = async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(200)
        .json({ status: "Failure", result: "User Not found" });
    } else {
